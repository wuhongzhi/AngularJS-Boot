define(['require', 'exports', 'application'], function(require, exports, application) {
    ...
    module.config(['$compileProvider', '$httpProvider', function($compileProvider, $httpProvider) {
        ...
        var isUndefined = angular.isUndefined, 
            defacl = {CREATE: 'C', READ: 'R', MODIFY: 'U', DELETE: 'D', CONFIRM: 'F', VERIFY: 'V', REJECT: 'J'};
        ...
        this.$get = ['$route', function($route) {
            ...
            return self = {
                ...
                canConfirm: function(uri) {
                    uri = uri || $route.current && $route.current.$$route.extras.path;
                    return self.hasPermission(uri, self.ACL.CONFIRM);
                },
                canVerfy: function(uri) {
                    uri = uri || $route.current && $route.current.$$route.extras.path;
                    return self.hasPermission(uri, self.ACL.VERIFY);
                },
                canReject: function(uri) {
                    uri = uri || $route.current && $route.current.$$route.extras.path;
                    return self.hasPermission(uri, self.ACL.REJECT);
                },
