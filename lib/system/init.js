                    ...
                    if (typeof(fn) === 'function' 
                        ...
                        var nfn = self[key] = (function() {
                            var oname = arguments[0],
                                keys = (verifys.keys[oname] = verifys.keys[oname] || {}),
                                dkey =  mname + '::' + key + '::' + oname;
                            if (keys[oname]) {
                                verifys.warns.push(key + '::' + oname + ' in module ' 
                                    + mname + ' was already defined in ' + keys[oname]);
                            } else {
                                keys[oname] = mname;
                                verifys.infos.push('define ' + key + '::' 
    ...
    function doBootstrap(rootModule) {
        ...
        (function load(modules) {
            if (phases.length) {
                ...
                if (!prodMode) {
                    verifys.infos.sort().forEach(function(v){console.debug(v)});
                    verifys.warns.sort().forEach(function(v){console.warn(v)});
                    uefi.progress("Application", "verify done", true);
                }
        ...
        function getRoutes(routes, module) {
            forEach(module.modules || {}, function(item, key) {
                var route = {
                    ...
                    "extras": {
                        "dynamic": prodMode ? false : item.dynamic,
                        "safe": item.safe,
                        "deps": item.depends,
                        "path": key
                    }
