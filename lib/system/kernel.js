define(['environment', 'version', 'cache', 'require'], function(environment, appversion, cache, require) {
    ...
    require([application.$$cacheUrl('json!lib/system/etc/kernel.json')], function(config) {
        ...
        require(kernelModules, function() {      
            define('application', angular.extend(application, {
            ...
            if (cached) {
                var $headElement = document.head || document.querySelector && document.querySelector('head');
                var $rootElement = angular.element($headElement);
                cache.$$style = function(style) {
                    $rootElement.append('<style>' + style + '</style>');
                };
            }
